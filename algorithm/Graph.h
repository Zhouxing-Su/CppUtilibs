...
*           3.  change T_DIST to T_WEIGHT and make the minimum value a negative number.
*/
...
protected:
    TopologicalGraph( unsigned vertexNumber, int minVertexIndex,
        Distance maxDist = ((std::numeric_limits<Distance>::max)() / static_cast<Distance>(2)),
        Distance minDist = static_cast<Distance>(0) );
...
TopologicalGraph<T_DIST>::TopologicalGraph( unsigned vn, int mvi, Distance maxDist, Distance minDist )
...
UndirectedGraph<T_DIST, DIST_MULTIPLICATION>::UndirectedGraph( const ArcList &arcList, unsigned vn, int mvi, Distance maxDist, Distance minDist )
    : TopologicalGraph( vn, mvi, maxDist, minDist ), multiplied( false )
{
...
// DirectedGraph =======================
template <typename T_DIST>
DirectedGraph<T_DIST>::DirectedGraph( const ArcList &arcList, unsigned vn, int mvi, Distance maxDist, Distance minDist )
    : TopologicalGraph( vn, mvi, maxDist, minDist )
{
    for (ArcList::const_iterator iter = arcList.begin(); iter != arcList.end(); iter++) {
        adjMat[iter->startVertex][iter->endVertex] = iter->dist;
    }
