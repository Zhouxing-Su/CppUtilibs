...
namespace szx
{
    ...
    public:
        ...
        int indexOf( int e ) const { return index[e - lowerBound]; }
    ...
    protected:
        int lowerBound;       // min value of elements
        int upperBound;       // max value of elements
    ...
    public:
        ...
        bool isElementExist( int e ) const { return (index[e - lowerBound] != INVALID_INDEX); }
        // if the element is in range [min, max], return true
        bool isElementValid( int e ) const { return ((e <= upperBound) && (e >= lowerBound)); }
